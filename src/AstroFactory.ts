...
export default class AstroFactory {
  static createSmallStar(scene: Scene) {
    ...
    const starParticleSystem = ParticleSystemSet.Parse(sunParticles, scene, true);
  ...
  static createPlanet(opts: PlanetOptions, scene: Scene) {
    const planet = MeshBuilder.CreateSphere(opts.name, { diameter: 1 }, scene) as Planet;
    ...
    const planetTrail = new TrailMesh(planet.name + '-trail', planet, scene, 0.1, circum, true);
    ...
    /* Animation */
    planet.orbitAnimationObserver = this.createAndStartOrbitAnimation(planet, scene);
